...
export function DraftEmail({ clientId, clientName, alertId, language }: DraftEmailProps) {
  ...
  if (!advisory && !loading) {
    return (
      <button
        onClick={generate}
        className="inline-flex items-center gap-2 rounded-lg border border-six-orange/40 bg-six-orange/10 px-3 py-2 text-xs font-medium text-amber-100 transition-colors hover:bg-six-orange/20"
      >
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900/40 px-3 py-3 text-xs text-slate-400">
        <Loader2 className="h-4 w-4 animate-spin text-six-orange" />
    ...
    <div className="rounded-lg border border-slate-700 bg-slate-900/50 p-3">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <span className="inline-flex items-center gap-1.5 text-xs font-medium uppercase text-slate-400">
          <Mail className="h-3.5 w-3.5 text-six-orange" />
          Email to {clientName}
        </span>
      ...
      {isEditing ? (
        <input
          ...
          className="mb-2 w-full rounded border border-slate-600 bg-slate-950/60 px-2 py-1.5 text-sm font-medium text-white outline-none focus:border-six-orange/60"
        />
      ) : (
      ...
      {isEditing ? (
        <textarea
          ...
          rows={12}
          className="w-full resize-y rounded border border-slate-600 bg-slate-950/60 px-2 py-2 text-sm leading-6 text-slate-200 outline-none focus:border-six-orange/60"
        />
      ) : (
